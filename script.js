...
youtubeInput.addEventListener('keydown', async (e) => {  
    if (e.key === 'Enter') {  
        ...
        try {  
            ...
            const res = await fetch('/upload_youtube', {  
                method: 'POST',  
                body: formData  
            });  
            ...
            if (data.job_id) {  
                alert('Видео отправлено! Job ID: ' + data.job_id);  
                youtubeInput.value = ''; // очищаем поле  
            } else {  
...
const youtubeInput = document.getElementById('youtube-url');
const youtubeSendBtn = document.getElementById('youtube-send');

youtubeSendBtn.addEventListener('click', async () => {
    const url = youtubeInput.value.trim();
    if (!url) return;
